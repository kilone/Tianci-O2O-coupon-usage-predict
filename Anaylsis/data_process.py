...
def get_source_data():
    # 源数据路径
    DataPath = r'C:\Users\KILO\DataAnalysis\TIANCI'
...
def get_offline_features(X, offline):
    ...
    # 领券的星期特征
    X['weekday'] = X.Date_received.dt.weekday + 1

    # 领券日期是否为周末
    X['weekday_type'] = X['weekday'].apply(lambda x: 1 if x in [6, 7] else 0)

    #领券日期的one_hot编码
    weekday_cols = [f'weekday_{i}' for i in range(1, 8)]
    temp1 = pd.get_dummies(X['weekday'].replace('null', np.nan))
    temp1.columns = weekday_cols
    X[weekday_cols] = temp1
    ...
    # 领取优惠券次数
    temp = temp.groupby('User_id').size().reset_index(name='u1')
...
if __name__ == '__main__':
    ...
    # 预处理后数据存放路径
    FeaturePath = 'data_preprocessed_3'
